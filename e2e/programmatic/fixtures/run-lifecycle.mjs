...
try {
  ...
  const concurrentCycles = await Promise.all([
    cycleRunner.run({ filters: ['failing.test.ts'], filterMode: 'exact' }),
    cycleRunner.run({ filters: ['sum.test.ts'], filterMode: 'exact' }),
  ]);
  ...
  console.log(
    `__RSTEST_API_RESULT__${JSON.stringify({
      ...
      concurrentCycles: concurrentCycles.map((cycle) => ({
        ok: cycle.ok,
        files: cycle.files.map((file) => file.testPath.split('/').pop()),
      })),
      blobRunnerError,
      errors,
      oneShotBuildFailure: {
