...
describe('programmatic createRstest', () => {
  ...
  it('supports reusable runners and contains build failures', async ({
    onTestFinished,
  }) => {
    const { cli } = await runRstestCli({
      command: 'node',
    ...
    expect(result.concurrentCycles).toEqual([
      { ok: false, files: ['failing.test.ts'] },
      { ok: true, files: ['sum.test.ts'] },
    ]);
