...
export interface RstestRunner {
  ...
  /**
   * Waits for accepted build and run work to settle before teardown. Calls are
   * idempotent and observe the same teardown result. Work requested after
   * close starts is rejected.
   */
  close(): Promise<void>;
}
